...
//			if($data['Allnew'] == 1){$used = "<img src='./images/allnew.gif'>";}
			...
			if($data['Transport'] == 1){
				...
				$price_info = "<div style='padding-top:2px;color:#FFFFFF; text-align:center; font-size:24px; line-height:50px; width:150px'>"  . $data['price_info'] . "</div>";
			}
			else{
			if($data['Price1'] > 9999){
				$price_info = <<<EOD
			...
			$WEB_CONTENT .= <<<EOD
				<tr>
					<td style="text-align:left; background:white">
						<div style="width:676px; height:279px; overflow:hidden; background:url('./images/bg_product5.png');">
								...
								<table width="676" height="279" cellpadding="0" cellspacing="0" border=0 style="{$activity}">
									...
									<tr>
										...
										<td style="vertical-align:top; padding-left:5px" valign="top" >
											<div style="width:218px; height:28px; margin-top:15px; border:solid 0px gray; overflow:hidden"><a href="service_product5_detail.php?tab={$tab}&no={$data['No']}&area={$area}&type={$type}&catalog={$catalog}" style="color:#000000; font-size:22px; font-weight:bold" title="{$data['Name']}：{$data['Description']}">{$name}</a></div>
