...
while($rs = mysql_fetch_array($result)){
	...
	if($rs['Question'] == 1){
		$rating .= <<<EOD
			...
			<tr style="background:{$bgcolor}; height:40px">
				<td style="text-align:left; color:blue">業者回覆：{$reply}</td>
				<td></td>
...
<script>
	...
	function postToFeed() {
		var caption = encodeURI("http://{$WEB_HOST}/service_product4_detail.php?no={$no}") ;

        var obj = {
			method: 'feed',
			link: 'http://{$WEB_HOST}/service_product4_detail.php?no={$no}',
            picture: '{$fb_thumb}',
            name: '{$fb_title}',
//            caption: caption,
            description: '{$fb_desc}'
...
if($data['L1'] > 0 && $data['L2'] > 0){
	...
	$path1="parent.Dialog1('direction.php?no={$data['No']}&start=1',500);";
	$path2="parent.Dialog1('direction.php?no={$data['No']}&start=2',500);";
	if($data['Status1'] != 3){
									...
									<table cellpadding="0" cellspacing="0" border="0" width="100%">
										<tr>
											...
											<td style="text-align:right">
												<input type="button" value="服務位置更新" onClick="setCenter();">
												<input type="button" value="業者遞送路徑" onClick="{$path1}">
												<input type="button" value="民眾前往路徑" onClick="{$path2}">
											</td>
										</tr>
									</table>
								</td>
...
EOD;
			if($data['Cashflow'] == 1){
				$onclick = ((!empty($_SESSION['member'])) ? "parent.Dialog('buynow.php?id={$data['No']}');":"window.location.href='member_login.php?url=" . urlencode("service_product4_detail.php?no={$data['No']}") . "';");
...
$url = "http://{$WEB_HOST}/";//service_product4_detail.php?no={$no}"
if($data['Mode'] == 2){
	if($data['Deliver'] == 0){
				...
				$question1 = <<<EOD
					...
					<tr>
						<td style="padding-top:22px;text-align:left">
							...
							<div class="fb-comments" data-href="http://{$WEB_HOST}/service_product4_detail.php?no={$no}" data-num-posts="3" data-width="612" data-order-by="reverse_time" reverse=1 simple=1></div>
...
EOD;
			...
			$info .= (($data['Url'] != "") ? "<tr><td valign='top' nowrap align='right'>業者網站：</td><td valign='top'><a href='" . $data['Url'] . "' target='_blank'>" . $data['Url'] : "") . "</a></td></tr>";
//			$info .= "<tr><td valign='top' nowrap align='right'>車行名號：</td><td valign='top'>({$taxi_company[$data['taxi_company']]})</td></tr>";
			$info .= (($data['Phone'] != "") ? "<tr><td valign='top' nowrap align='right'>聯絡電話：</td><td valign='top'>" . $data['Phone'] : "") . "</td></tr>";
			$info .= (($data['Receipt'] != "") ? "<tr><td valign='top' nowrap align='right'>發票或收據：</td><td valign='top'>" . $receipt[$data['Receipt']] : "") . "</td></tr>";
										...
										<tr>
											...
											<td style="width:300px; text-align:center; vertical-align:bottom" valign="bottom">
												<table align="center" width="100%" cellpadding="0" cellspacing="0">
													...
													<tr>
														<td style="font-size:14px; height:47px">服務詳情請參考以下之說明介紹</td>
													</tr>
												</table>
											</td>
										</tr>
