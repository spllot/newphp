...
while($rs = mysql_fetch_array($result)){
	...
	$caption = "<table cellpadding=0 cellspacing=0 border=0 width=143><tr><td><span class='icon-sale'>" .
	(float)(number_format($rs['Discount'],1)) . "折</span></td></tr><tr><td><div class='title' title='{$rs['Caption']}'>{$rs['Caption']}</div></td></tr></table>";
	
	$ad2 .= "<td align='center' valign='top'>
	<div class='item'>
		<div style='width:120px; height:70px; overflow:hidden; border:solid 1px #CCCCCC'><a href='{$rs['Url']}' target='" . (($tmp == "http://") ? "_blank" : "") . "' style='color:#D6CF00'><img src='{$pics}' border='0' style='width:120px; height:70px;'></a></div>
...
while($rs = mysql_fetch_array($result)){
	...
	$ad2 .= "<td align='center' valign='top'>
	<div class='item'>
		<div style='width:120px; height:70px; overflow:hidden; border:solid 1px #CCCCCC'><a href='{$ur}' target='" . (($tmp == "http://") ? "_blank" : "") . "' style='color:#ca2627; text-decoration:none'><img src='./upload/{$rs['Photo']}' border='0' style='width:120px; height:70px;'></a></div>
...
<div id="menu-recommend">
	<div class="header"><h1>即時服務推薦</h1></div>
	<table class="list">
		<tr>
			<td class="outer">
				<div id="scrollbarDemo2"><?=$ad2?></DIV>
			</td>
		</tr>
	</table>
